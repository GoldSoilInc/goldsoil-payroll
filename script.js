...
function renderResults(entries, period) {
  ...
  html += '<table class="summary-table"><thead><tr><th>Person</th><th>Role</th><th>Lines</th><th class="num">Total</th></tr></thead><tbody>';
  for (const [person, v] of sortedPeople) {
    html += `<tr><td>${escapeHTML(person)}</td><td>${escapeHTML(v.role)}</td><td class="num">${v.count}</td><td class="num">${fmtUSD(v.total)}</td></tr>`;
  }
  html += `<tr class="grand-total"><td colspan="3"><strong>TOTAL</strong></td><td class="num"><strong>${fmtUSD(grandTotal)}</strong></td></tr>`;
  html += '</tbody></table>';
  ...
  for (const [person, v] of sortedPeople) {
    ...
    detailHTML += `<details class="person-detail" open><summary><span>${escapeHTML(person)}</span><span class="role-tag">${escapeHTML(v.role)}</span><span class="num">${fmtUSD(v.total)}</span></summary>`;
    detailHTML += '<table class="detail-table"><thead><tr><th>Source</th><th>Type</th><th>Calculation</th><th>Notes</th><th class="num">Amount</th></tr></thead><tbody>';
    for (const e of personEntries) {
      detailHTML += `<tr class="flag-${e.flag.toLowerCase()}"><td>${escapeHTML(e.source)}</td><td>${escapeHTML(e.type)}</td><td>${escapeHTML(e.calc)}</td><td>${escapeHTML(e.notes)}</td><td class="num">${fmtUSD(e.amount)}</td></tr>`;
    }
