...
function renderResults(entries, period) {
  ...
  const renderOne = (person, v) => {
    ...
    // Per-person manual review list. Grouped by (type, note) so duplicate
    // notes (e.g. LPA's QoC verification reminder repeated across N lines)
    ...
    if (reviewItems.length > 0) {
      ...
      h += `<div class="person-review-list">`;
      h += `<div class="person-review-h">Manual Review Needed (${totalReview})</div>`;
      h += `<ul>`;
      for (const g of groups) {
        ...
        h += `<li><strong>${escapeHTML(g.type)}</strong>${sources}: ${escapeHTML(g.message)}</li>`;
      }
      h += `</ul></div>`;
    }
...
/* ------------------------------------------------------------
   ...
   ----------------------------------------------------------------
   Per-person summary: one row per Active roster member with their
   final total. Includes $0 rows for active people with no commission
   this period, matching what's shown on the dashboard so payroll can
   confirm "checked, nothing owed" for everyone in one paste.
   ------------------------------------------------------------ */
function copyTSV(entries) {
  // Aggregate by person from entries (skip INELIGIBLE and unattributed).
  const byPerson = {};
  for (const e of entries) {
    if (e.flag === 'INELIGIBLE') continue;
    if (!e.person || e.person === '—') continue;
  ...
  // Add $0 rows for Active roster members not in entries, matching the
  // case-insensitive logic in renderResults so people don't get listed twice.
  const withEntries = new Set();
  for (const personName of Object.keys(byPerson)) {
    const norm = normalizeName(personName);
    if (PEOPLE[norm]) {
...
function buildEmailPayload(entries, period) {
  ...
  for (const e of entries) {
    if (e.flag === 'INELIGIBLE') continue;
    if (!e.person || e.person === '—') continue;
    if (!byPerson[e.person]) byPerson[e.person] = { total: 0, byType: {} };
    byPerson[e.person].total += e.amount;
    const t = e.type || '(unknown)';
    byPerson[e.person].byType[t] = (byPerson[e.person].byType[t] || 0) + e.amount;
  }
  ...
  for (const [name, info] of Object.entries(byPerson)) {
    ...
    recipients.push({ name, email, total: info.total, breakdown });
  }
...
function renderEmailPreviewHTML(r, periodLabel) {
  ...
  for (const b of r.breakdown) {
    rows += '<tr><td style="padding:6px 12px 6px 0;color:#4A5468;">' + escapeHTML(b.type) + '</td>'
         +  '<td style="padding:6px 0;text-align:right;font-family:monospace;">' + fmtUSD(b.amount) + '</td></tr>';
  }
  return '<div style="font-family:sans-serif;color:#0F1B2D;line-height:1.55;font-size:14px;padding:1rem;background:white;border-radius:2px;">'
    + '<p>Hi ' + firstName + ',</p>'
    + '<p>Your commission for <strong>' + escapeHTML(periodLabel) + '</strong> is <strong style="color:#2E5D3D;">' + fmtUSD(r.total) + '</strong>.</p>'
    + '<p style="margin:24px 0 8px;font-size:12px;text-transform:uppercase;letter-spacing:0.08em;color:#4A5468;">Breakdown</p>'
    + '<table style="border-collapse:collapse;max-width:380px;">' + rows
    + '<tr style="border-top:2px solid #D7CFBF;"><td style="padding:10px 12px 8px 0;font-weight:600;">Total</td>'
    + '<td style="padding:10px 0 8px;text-align:right;font-family:monospace;font-weight:600;">' + fmtUSD(r.total) + '</td></tr>'
    + '</table>'
